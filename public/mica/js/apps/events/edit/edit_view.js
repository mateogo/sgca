DocManager.module("EventsApp.Edit", function(Edit, DocManager, Backbone, Marionette, $, _){
  ...
  Edit.Editor = Marionette.ItemView.extend({
    ...
    onRender: function(){
      ...
      self.validateLocation(self.model.get('location')).then(function(data) {
        self.form.render();

        self.$el.html(self.form.el);

        self.form.fields.espacios.editor.setOptions(data.spaces);

        self.attachView = new AppCommon.AttachmentView({el:self.$el.find('#attachmentContainer'),model:self.model});
        self.attachView.render();

        self.$el.find('[name=headline]').focus();

        self.validateFechaType();
        self.validateDates();
      });
    },

    validateLocation: function(location_id){
      var dfd = $.Deferred();
      dfd.resolve({});
      return dfd.promise();

      var locacionSelected = location_id || this.$el.find('[name=location]').val();
      var form = this.form;
    ...
    onLocacionAgregada: function(){
      this.form.fields.location.editor.setOptions(new DocManager.Entities.LocationCollection());
    },
