/*
 *  calendar salonsuscriptions.js
 *  package: /calendar/controllers
 *  DOC: 'salonsuscriptions' collection controller
 *  Use:
 *     Exporta el objeto controller de un salonsuscription via 'exports'
 *     metodos exportados:
...
var BSON;
var config = {};
var salonsuscriptionsCol = 'salonsuscriptions';
...
//ATENCION: para agregar un serial, agregar entrada en tsalonprofile_adapter y en series;
var series = ['salon2016','salon9999'];

var seriales = {};

//ATENCION: revisar el criterio de seleccion de registro en setNodeCode()
var tsalonsuscription_adapter = {
    salon:{
        serie: 'salon2016',
        base: 100000,
        prefix: 'SN-16'
...
var addSerial = function(serial,data){
    ...
    //console.log('addSerial:salonsuscription.js INIT con exito: [%s] next:[%s]',seriales[serial].serie,seriales[serial].nextnum);
};
...
var setNodeCode = function(node){
    var adapter = tsalonsuscription_adapter[node.tregistro] || tsalonsuscription_adapter['poromision'];
...
var fetchOne = function(query, cb) {
    dbi.collection(salonsuscriptionsCol, function(err, collection) {
...
var insertNewProfile = function (req, res, salonsuscription, cb){

    dbi.collection(salonsuscriptionsCol).insert(salonsuscription,{w:1}, function(err, result) {
            if (err) {
                if(res){
...
exports.findByQuery = function(req, res) {
    ...
    //console.log('find:salonsuscription Retrieving salonsuscription collection with QUERY [%s] [%s]', page, limit);

    cursor = dbi.collection(salonsuscriptionsCol).find(query).sort({cnumber:1});
    if(textsearch){
...
exports.findAll = function(req, res) {
    console.log('findAll: Retrieving all instances of [%s] collection', salonsuscriptionsCol);
    dbi.collection(salonsuscriptionsCol, function(err, collection) {
...
exports.add = function(req, res) {
    console.log('add:salonsuscription.js: NEW RECEIPT BEGINS');
    var salonsuscription = req.body;
    addNewProfile(req, res, salonsuscription);
};

exports.update = function(req, res) {
    var id = req.params.id;
    var salonsuscription = req.body;
    delete salonsuscription._id;
    console.log('Updating node id:[%s] ',id);
    console.log(JSON.stringify(salonsuscription));
    dbi.collection(salonsuscriptionsCol, function(err, collection) {
        collection.update({'_id':new BSON.ObjectID(id)}, salonsuscription, {safe:true}, function(err, result) {
            if (err) {
                console.log('Error updating %s error: %s',salonsuscriptionsCol,err);
                res.send({error: MSGS[0] + err});
            } else {
                console.log('UPDATE: se insertaron exitosamente [%s] nodos',result);
                res.send(salonsuscription);
            }
        });
    });
};
...
exports.partialupdate = function(req, res) {
    ...
    dbi.collection(salonsuscriptionsCol).update(query, {$set: update}, {safe:true, multi:true}, function(err, result) {
        if (err) {
            console.log('Error partial updating %s error: %s',salonsuscriptionsCol,err);
            if(res){
                res.send({error: MSGS[0] + err});
            }else if(cb){
                cb({error: MSGS[0] + err});
