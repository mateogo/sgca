      ...
      Solicitud.findById(raw._id,function(err,result){
        if(result && result.get('owner_id')){
          ...
          if(owner_id.toString() != user._id.toString()){
            res.send(422,'Solo el creador puede modificar la solicitud');
            return;
          }
        }
        ...
        var initWorkFlow = false;
        if(obj.isNew() || !obj.get('owner_id')){
        ...
        obj.save(function(err,result){
          ...
          var workflow = new ObrasWorkflowService(user);
          if(initWorkFlow){
            workflow.iniciarSolicitud(obj,function(err,token){
              res.json(result);
            });
          }else{
    ...
    remove: function(req,res){
      ...
      Solicitud.findById(id,function(err,object){
        ...
        if(object && object.get('owner_id') != user._id){
          res.status(422).send('Solo el creador puede borrar la solicitud');
          return;
        }
